...
def parser():
    ...
    parser.add_argument(
        "-p", "--process", required=True, help=f"Process to be run. {processes}"
    )
    ...
    parser.add_argument(
        "-k", "--kernel", required=False, help="Kernel to be slided over an image"
    )
    parser.add_argument(
        "-s",
        "--strength",
...
def clahe(img, clip_limit=2.0, tile_grid_size=8, apply=1):
...
def get_dark_channel(img):
    ...
    return cv2.erode(
        dark, kernel
    )  # erode filter will remove small white elements from the dark channel (probably noise)
    # return dark
...
def get_atmospheric_light(img, dark):
    ...
    return (np.array(img_vec[position])).reshape(
        1, 3
    )  # return RGB values corresponding to the brightest grey pixel - it is assumed it is the atmospheric light
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Test denoising:
            ...
            cv2.imshow(
                f"Denoised with strength = {strength}", np.hstack([img, modified])
            )
            cv2.imwrite("modified.jpg", modified)
            cv2.waitKey(0)
